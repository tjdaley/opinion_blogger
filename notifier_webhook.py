...
async def _run_retry(case_number: str):
    ...
    try:
        ...
        if not row:
            notifier.send(f"retry {case_number}: case not found")
            return
        ...
        notifier.send(f"retry {case_number} -> {new_status}")
    except Exception as e:
        logger.exception("retry failed for %s", case_number)
        notifier.send(f"retry {case_number} error: {e}")
...
async def inbound_sms(request: Request, background_tasks: BackgroundTasks):
    ...
    if cmd == "status":
        return _twiml(notifier.status_summary())
